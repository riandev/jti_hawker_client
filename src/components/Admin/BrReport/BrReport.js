...
const BrReport = () => {
  ...
  const handleBrReport = () => {
    fetch(`http://192.168.10.11:5051/baReport/${brDate}`)
      .then((res) => res.json())
      .then((data) => setStatus(data));
  };
  const handleTerritoryReport = () => {
    fetch(`http://192.168.10.11:5051/territoryReport/${territoryDate}`)
      .then((res) => res.json())
      .then((data) => setTerritoryStatus(data));
  };
  const handleAreaReport = () => {
    fetch(`http://192.168.10.11:5051/areaReport/${areaDate}`)
      .then((res) => res.json())
      .then((data) => setAreaStatus(data));
  };
  const handleRegionReport = () => {
    fetch(`http://192.168.10.11:5051/regionReport/${regionDate}`)
      .then((res) => res.json())
