...
const LeadGenerate = () => {
  ...
  const generateInitial = () => {
    fetch("http://192.168.10.12:5051/initialLead?initDate=" + initialDate)
      .then((res) => res.json())
      .then((data) => {
        console.log(data);
        setInitialLeads(
  ...
  const updateInitialLeads = () => {
    fetch("http://192.168.10.12:5051/updateInitialLead", {
      method: "PATCH",
  ...
  const regenerateLeads = () => {
    fetch("http://192.168.10.12:5051/regenerate?regenDate=" + regenDate)
      .then((res) => res.json())
      .then((data) => {
        console.log(data);
        setRegenerate(
  ...
  const regenerateUpdate = () => {
    fetch("http://192.168.10.12:5051/regenerateUpdate", {
      method: "PATCH",
      headers: { "Content-Type": "application/json" },
